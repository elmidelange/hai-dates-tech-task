...
def check_format(date_str: str) -> None:
    ...
    if (is_leap_year(year) == False) and (month == 2) and (day == 29):
        raise ValueError(
...
def is_leap_year(year: int) -> bool:
    ...
    ASSUMPTION: To be a leap year, the year number must be divisible by four, except for end-of-century years, which must be divisible by 400.
...
def get_day_month_year(date_str: str) -> Tuple[int, int, int]:
...
def get_total_days(year: int, month: int, day: int) -> int:
    ...
    num_days += count_leap_years(year, month, day)

    return num_days


def count_leap_years(year: int, month: int, day: int) -> int:
